...
f = open('../data/NNLO_bayes_factor.csv','r')
next(f)
fig = plt.figure('Bayes Factors',figsize=fig_size2)
...
ax.set_xlabel(r'prior width', fontsize=fs_xy)
ax.xaxis.set_tick_params(labelsize=ts,width=lw)
ax.yaxis.set_tick_params(labelsize=ts,width=lw)
[ax.spines[i].set_linewidth(lw) for i in ax.spines]

plt.draw()
plt.savefig('./NNLO_bayesfactor.pdf',transparent=True)
plt.show()
